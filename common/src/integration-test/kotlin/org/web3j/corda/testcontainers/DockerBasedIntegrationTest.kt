...
open class DockerBasedIntegrationTest {
    ...
    private fun getCertificate(node: File) {
        ...
        NetworkMap.build(CordaService("http://localhost:${NETWORK_MAP.getMappedPort(8080)}")).apply {
    ...
    private fun updateNotaryInNetworkMap(nodeInfoPath: String) {
        var networkMapApi = NetworkMap.build(CordaService("http://localhost:${NETWORK_MAP.getMappedPort(8080)}"))
        val loginRequest = LoginRequest("sa", "admin")

        val token = networkMapApi.admin.login(loginRequest)
        networkMapApi = NetworkMap.build(CordaService("http://localhost:${NETWORK_MAP.getMappedPort(8080)}"), token)
